...
def resource_path(filename):
    """Return a resource path in development or inside the macOS app bundle."""
    if getattr(sys, "frozen", False):
        # PyInstaller data files are exposed through _MEIPASS.
...
def configure_bundled_playwright():
    """Use Chromium bundled in Student Roster.app when running packaged."""
    ...
    candidates = [
        os.path.join(contents_dir, "Resources", "ms-playwright"),
        os.path.join(exe_dir, "ms-playwright"),
        os.path.join(getattr(sys, "_MEIPASS", ""), "ms-playwright"),
    ]

    for browser_dir in candidates:
        if browser_dir and os.path.isdir(browser_dir):
            os.environ["PLAYWRIGHT_BROWSERS_PATH"] = browser_dir
            return

    raise RuntimeError(
        "Bundled Chromium could not be found inside Student Roster.app."
    )
...
class StudentRosterGUI(ctk.CTk):
    def __init__(self):
        ...
        self._load_settings()
        self.term_var = tk.StringVar(value=self.saved_term)
    ...
    def _load_settings(self):
        self.saved_term = "Fall 2026"
        try:
            with open(SETTINGS_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
            value = str(data.get("last_term", "")).strip()
            if value:
                self.saved_term = value
        except Exception:
            pass
    ...
    def _save_settings(self):
        try:
            ...
            with open(SETTINGS_FILE, "w", encoding="utf-8") as f:
                json.dump({"last_term": self.term_var.get().strip()}, f, indent=2)
        except Exception:
            pass
    ...
    def _build_ui(self):
        ...
        try:
            ...
            ctk.CTkLabel(header, text="", image=self.logo).grid(
        ...
        ctk.CTkLabel(
            ...
            font=ctk.CTkFont(family="Helvetica Neue", size=27, weight="bold")
        ...
        label_font = ctk.CTkFont(family="Helvetica Neue", size=13, weight="bold")
        entry_font = ctk.CTkFont(family="Helvetica Neue", size=14)
        help_font = ctk.CTkFont(family="Helvetica Neue", size=12)
        ...
        ctk.CTkLabel(
            card,
            text="Example: 43422, 43378, 43445 — separate multiple sections with commas",
            font=help_font
    ...
    def _worker(self):
        ...
        try:
            with contextlib.redirect_stdout(writer), contextlib.redirect_stderr(writer):
                result = run_roster(
                    self.term_var.get(), self.sections_var.get(), self.selected_csv)
    ...
    def open_folder(self):
        if self.last_csv:
            subprocess.run(["open", os.path.dirname(self.last_csv)], check=False)
    ...
    def show_about(self):
        messagebox.showinfo(
            ...
            "on this Mac so you do not have to sign in on every run."
        )
